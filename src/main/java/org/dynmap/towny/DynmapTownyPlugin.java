...
public class DynmapTownyPlugin extends JavaPlugin {
	...
	private static class AreaStyle {
		...
		AreaStyle(FileConfiguration cfg, String path, MarkerAPI markerapi) {
			...
			pvpfillopacity = cfg.getDouble(path+".pvpFillOpacity", -1);
		...
		public int getPVPFillColor(AreaStyle cust, AreaStyle nat, TownBlockType btype) {
			if((cust != null) && (cust.pvpfillcolor >= 0))
				return cust.pvpfillcolor;
			else if((nat != null) && (nat.pvpfillcolor >= 0))
				return nat.pvpfillcolor;
			else if(pvpfillcolor >= 0)
				return pvpfillcolor;
			else
				return 0xFF0000;
		}
		...
		public double getPVPFillOpacity(AreaStyle cust, AreaStyle nat) {
			if((cust != null) && (cust.pvpfillopacity >= 0))
				return cust.pvpfillopacity;
			else if((nat != null) && (nat.pvpfillopacity >= 0))
				return nat.pvpfillopacity;
			else if(pvpfillopacity >= 0)
				return pvpfillopacity;
			else
				return 0.35;
		}
	...
	private void addStyle(Town town, String natid, AreaMarker m, TownBlockType btype) {
		String resid = town.getName();
